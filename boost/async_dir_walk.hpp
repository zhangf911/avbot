...
public:
	...
	async_dir_walk( boost::asio::io_service & _io_service, boost::filesystem::path path, DirWalkHandler _dir_walk_handler )
		: io_service( _io_service ), dir_it_cur(path), dir_walk_handler(_dir_walk_handler)
	{
...
template<class DirWalkHandler>
async_dir_walk<DirWalkHandler> make_async_dir_walk_op( boost::asio::io_service & io_service, boost::filesystem::path path, DirWalkHandler dir_walk_handler)
{
	return async_dir_walk<DirWalkHandler>( io_service, path, dir_walk_handler);
}
...
{
	detail::make_async_dir_walk_op(io_service, path,  dir_walk_handler);
}
