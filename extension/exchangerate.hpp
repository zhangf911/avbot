...
struct EOR{
  ...
  std::string list;
		...
		{ u8"台币人民币", "TWDCNY" },
		{ u8"美元澳门元", "USDMOP" },
		{ u8"美元", "USDCNY" },	// !!!单一币种缺省查与人民币的汇率，必须放在最后，避免正则匹配干扰其他项
		{ u8"澳元", "AUDCNY" },
		{ u8"加元", "CADCNY" },
		{ u8"瑞郎", "CHFCNY" },
		{ u8"欧元", "EURCNY" },
...
struct exchangerate_fetcher_op{
	  ...
	  : io_service(_io_service), sender(_sender), stream(new avhttp::http_stream(_io_service)), money(_money), buf(boost::make_shared<boost::asio::streambuf>())
	{
		...
		if (list.empty()){
			sender( std::string( money + u8" 无汇率数据或avbot暂不支持"));
		}else{
	...
	void operator()(boost::system::error_code ec, std::size_t bytes_transfered)
	{
		if (!ec || ec == boost::asio::error::eof){
			...
			{
				std::string msg = boost::str(boost::format(u8"%s 当前汇率 %s 昨收 %s 今开 %s 最高 %s 最低 %S 买入 %s 卖出 %s ")
