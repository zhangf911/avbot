...
{
	preamble_irc_fmt = u8"%a 说：";
	preamble_qq_fmt = u8"qq(%a)说：";
	preamble_xmpp_fmt = u8"(%a)说：";
...
void avbot::callback_on_qq_group_newbee(webqq::qqGroup_ptr group, webqq::qqBuddy_ptr buddy)
{
	...
	message.add("preamble", u8"群系统消息: ");

	if (buddy){
		message.add("message.text", boost::str(boost::format(u8"新人 %s 入群.") % buddy->nick ));
	}else{
		message.add("message.text", u8"新人入群,  可是 webqq 暂时无法获取新人昵称.");
	}
